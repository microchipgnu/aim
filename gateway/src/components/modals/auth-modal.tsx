...
export function AuthModal({ onSuccess, trigger }: AuthModalProps) {
    ...
    const { session, user, isLoading, updateSessionAndUser, apiKeys, createApiKey } = useAuth();
                                ...
                                <CardContent className="space-y-4">
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <Button variant="outline" size="icon" onClick={createApiKey}>
...
function SignUpForm({ onSuccess }: { onSuccess?: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            await authClient.signUp.email({ email, password, name, image: undefined })
            onSuccess?.();
        } catch (error) {
            console.error(error);
        } finally {
