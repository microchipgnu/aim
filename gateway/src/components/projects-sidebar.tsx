...
export function ProjectsSidebar({ backPath, children, routes, projectId }: SidebarProps) {
    ...
    const handleCreateFile = async () => {
        try {
            await createDocumentAction({
                ...
                path: nanoid() + ".md",
                content: "Hello, world! ❌",
                userId: await authClient.getSession().then(session => session?.data?.user?.id || "")
            })
    ...
    const renderFolderContent = (structure: FolderStructure, basePath: string = '') => {
            ...
            if (content.files.length > 0) {
                return (
                    <SidebarMenuItem key={currentPath}>
                        <SidebarMenuButton asChild isActive={pathname === content.files[0].path}>
                            <Link
            ...
            <Sidebar>
                    ...
                    <div className="flex h-12 divide-x">
                        ...
                        >
                            <div className="flex items-center gap-3 text-foreground">
                                <span className="text-lg font-semibold tracking-tight">AIM</span>
                            </div>
                        </Link>
                        {backPath && (
                            <Link
                ...
                <SidebarContent>
                    <ScrollArea className="h-[calc(100vh-8rem)]">
                        <SidebarMenu>
                            {renderFolderContent(folderStructure)}
                        </SidebarMenu>
                    </ScrollArea>
                </SidebarContent>
