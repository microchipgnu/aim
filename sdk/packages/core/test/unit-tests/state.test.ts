...
describe("StateManager", () => {
    test("constructor initializes with correct default state", () => {
        const manager = new StateManager({ variables: {} }, {
            plugins: [],
            adapters: [],
            variables: {},
            input: {},
    ...
    test("setData correctly updates scoped data", () => {
        ...
        expect(manager.getScopedVariables(GLOBAL_SCOPE)).toEqual({ foo: "bar", baz: "qux" });
    });
    ...
    test("addToTextRegistry adds text to correct scope", () => {
        ...
        expect(manager.getScopedText(GLOBAL_SCOPE)).toEqual(["test text", "more text"]);
    });
