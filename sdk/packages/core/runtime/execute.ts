...
async function handleAbortError(error: unknown, stateManager: any) {
    const isAbortError = (jsEnvironment.isBrowser && error instanceof DOMException && error.name === "AbortError") ||
...
async function* processResults({ node, stateManager, signal }: AIMRuntime & { signal: AbortSignal }) {
