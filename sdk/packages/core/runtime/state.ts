...
export class StateManager {
    ...
    constructor(initialConfig: AIMConfig, public runtimeOptions: RuntimeOptions) {
        ...
        this.stateUpdates$.pipe(
            scan((chain: StateBlock[], update: { state: RuntimeContext; action: string; }) => {
                const prevState = chain[chain.length - 1]?.state || initialContext;
                const newBlock = this.createStateBlock(prevState, update.state, update.action);
    ...
    private createRuntimeMethods(): RuntimeMethods {
        return {
            ...
            setData: async ({ data, scope }: { data: Record<string, any>, scope: string }) => this.setData(data, scope),
            resetContext: async () => this.resetContext(),
            addToTextRegistry: async ({ text, scope }: { text: string, scope: string }) => this.addToTextRegistry(text, scope),
            clearTextRegistry: async ({ scope }: { scope: string }) => this.clearTextRegistry(scope),
    ...
    pushStack(frame: StackFrame) {
        ...
        const existingFrameIndex = currentContext.stack.findIndex((f: { id: string; scope: string; }) => 
    ...
    popStack(scope: string) {
        const currentContext = this.runtimeContext$.value;
        const newStack = currentContext.stack.filter((frame: { scope: string; }) => frame.scope !== scope);
    ...
    addToTextRegistry(text: string, scope: string) {
        ...
        this.updateContext({ ...currentContext, textRegistry: newRegistry }, 'addToTextRegistry');
    }
    ...
    clearTextRegistry(scope: string) {
        ...
        this.updateContext({ ...currentContext, textRegistry: newRegistry }, 'clearTextRegistry');
    }
    ...
    private mergePluginConfig(plugin: AIMPlugin, currentConfig: AIMConfig): AIMConfig {
    ...
    private createStateBlock(prevState: RuntimeContext, newState: RuntimeContext, action: string): StateBlock {
        return {
            timestamp: Date.now(),
            action,
    ...
    getStateAtBlock(hash: string): RuntimeContext | undefined {
        return this.stateChain$.value.find((b: { hash: string; }) => b.hash === hash)?.state;
    }
}

// Export a factory function to create new state managers
export function createStateManager(initialConfig: AIMConfig, options: RuntimeOptions): StateManager {
