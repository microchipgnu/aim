...
export async function put(
  filename: string,
  body: Blob | ReadableStream | string | Uint8Array,
  options: {
  ...
  try {
    ...
    if (body instanceof Blob) {
      finalBody = body;
      finalContentType =
        finalContentType || body.type || 'application/octet-stream';
...
export async function listBucketContents(
  ...
  Array<{ key: string; size: number; lastModified: Date; path: string }>
> {
  try {
