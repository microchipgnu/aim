...
export const uploadFile = async (
  ...
  try {
    ...
    const validFiles = files.filter((file) => {
      ...
      return (
        file.buffer.length > 0 && file.mimetype !== 'application/octet-stream'
      );
    });
    ...
    if (validFiles.length === 0) {
      throw new AppError(
        400,
        'No valid files found - directories cannot be uploaded directly',
      );
    }
    ...
    const results = await Promise.all(
      validFiles.map(async (file) => {
        ...
        return await storage.put(relativePath, file.buffer, {
...
export const getSignedDownloadUrl = async (
  ...
  try {
    ...
    const url = await storage.getDownloadUrl(
      objectKey,
      typeof expiresIn === 'string' ? Number.parseInt(expiresIn) : undefined,
    );
    res.json({ url });
  } catch (error) {
    next(error);
...
export const clearBucketHandler = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
