...
import { aim, renderers } from '../core/index';
...
import ora from 'ora';
import { defaultRuntimeOptions } from '../core/aim';
  ...
  .action(async (originalPath: string) => {
    try {
      ...
      const getUserInput = async (schema: Array<{ name: string }>) => {
        ...
        try {
          for (const { name } of schema) {
            variables[name] = await question(
              chalk.cyan(`Enter value for ${name}: `),
            );
          }
        } finally {
          readline.close();
      ...
      const executeDocument = async (
        ...
        for await (const result of document.executeWithGenerator({
          input: variables,
        })) {
