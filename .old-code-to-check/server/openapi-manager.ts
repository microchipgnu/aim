...
export class OpenAPIManager {
  ...
  generateOpenAPISpec() {
    ...
    this.routes.forEach((route) => {
      const path =
        '/' + route.path.replace(/^api\//, '').replace(/\[(\w+)\]/g, '{$1}');
  ...
  generateAIPlugin() {
    return {
      ...
      description_for_human:
        'Plugin for interacting with the AIM server, allowing execution of AIM documents and management of requests.',
      description_for_model:
        'API for executing AIM documents, managing request lifecycles, and retrieving document information. Supports Server-Sent Events (SSE) for real-time updates.',
      auth: {
        type: 'none',
      },
      api: {
  ...
  private generatePathOperations(
    path: string,
    operationId: string,
    parameters: any[],
  ) {
