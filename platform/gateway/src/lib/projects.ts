import { db } from "@/db";
import { documents, documentVersions, projects, versionTags } from "@/db/schema";
...
export async function updateDocument(input: UpdateDocumentInput) {
    return await db.transaction(async (tx) => {
        ...
        const currentVersion = document.currentVersionId ? await tx
            .select()
...
export async function deleteDocument(documentId: string) {
    return await db.transaction(async (tx) => {
        ...
        await tx.delete(versionTags)
            .where(eq(versionTags.versionId, documentId));
...
export async function revertToVersion(documentId: string, versionId: string, userId: string) {
    return await db.transaction(async (tx) => {
        ...
        const revertVersion = await tx.insert(documentVersions).values({
            id: nanoid(),
            documentId: document.id,
...
export async function createVersionTag(name: string, versionId: string, userId: string) {
    ...
    return await db.insert(versionTags).values({
        id: nanoid(),
        name,
        versionId,
...
export async function updateDocumentPath(input: UpdateDocumentPathInput) {
    return await db.transaction(async (tx) => {
        ...
        const newVersion = await tx.insert(documentVersions).values({
            ...
            version: (currentVersion?.version ?? 0) + 1,
            message: input.message ?? `Renamed from ${document.path} to ${input.newPath}`,
            createdBy: input.userId,
            createdAt: new Date()
        }).returning().then(rows => rows[0]);
